...
Expect **154 failures** with Docker stopped, all from these two suites. That figure is **derived**
(97 + 57 by construction), not measured — the suites were last run with Docker up:

| Suite | Total | Fails without Docker |
|---|---|---|
| `IndyPOS.MigrationTool.Tests` | 119 | **57** (of the other 62, see Trap 3) |
...
So `IndyPOS.MigrationTool.Tests`'s 119 break down as: **57** need Docker, **24** need those real
...
discovers **99**, not 119 (**derived** as 119 − 20, not measured — the same 20-row collapse described
...
— **602 total** (601 pass, 1 skipped) — measured 2026-08-17. Without those databases the total is
**582** (**derived** as 602 − 20, not measured), still all green:

| Suite | Tests |
|---|---|
| `IndyPOS.Application.Tests` | 298 |
| `IndyPOS.StoreHub.IntegrationTests` | 104 (Docker) |
| `IndyPOS.MigrationTool.Tests` | 119 (Docker; 1 skipped. **99** without the real store data — Trap 3, derived) |
...
> artefacts dumped from real stores. Defects 2, 3, 4, 5, 9, 10, 11, 12, 13, 14, 15, 16, 17 and 18 are
> fixed (20 is open — a payment whose invoice is missing is dropped silently);
