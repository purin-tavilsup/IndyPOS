...
| 2 | **SQLite → PostgreSQL migration hardening** | 🚧 **IN PROGRESS** — test harness landed; defects 2, 3, 4, 9, 10, 11, 12, 13, 14 fixed (plus 7a); 5/6/8 pinned; **7b open and deliberately unpinned**; **15 new** (`verify` crashes on 2 of 3 stores). **Stock is now observable and correctable at the till** — see below | 3 divergent legacy schemas. The risky half of the rollout |
...
| 15 | **`MigrationVerifier` queries `PayLater` unconditionally** (`MigrationVerifier.cs:70`) — the same shape as defect 3, which was fixed only in `SqliteMigrationService` | **`verify` crashes with `no such table: PayLater` on MimyMart and MimyShop — 2 of the 3 real stores.** So the per-product stock check added for defect 14 (the one thing that proves migrated stock is right) has never been runnable on either of them. Found 2026-08-10 by running `verify` for real against a migrated MimyShop fixture; `migrate` itself succeeds, only `verify` dies. Confirmed pre-existing on `development` | ❌ **new**, unpinned |
